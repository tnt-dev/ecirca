...
get(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    idx = get_index(ctx, i);
    if (ctx->circa[idx] == (18446744073709551615UL)) {
        ret = enif_make_atom(env, "empty");
    } else {
        ret = enif_make_ulong(env, ctx->circa[idx]);
    }
    return enif_make_tuple(env,2,enif_make_atom(env, "ok"),ret);
}
...
set(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    if (!enif_get_ulong(env, argv[2], &val_big)) {
        ...
        if (val_big > 576460752303423487) {
           return enif_make_tuple(env,2,enif_make_atom(env, "ok"),enif_make_atom(env, "overflow"));
        }
        val = (elem_t) val_big;
    }
    ...
    return enif_make_tuple(env,2,enif_make_atom(env, "ok"),enif_make_tuple(env,2,enif_make_ulong(env, old_val),enif_make_ulong(env, new_val)))
                                                               ;
}
...
update(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    else if (val != (18446744073709551615UL) && ctx->circa[idx] != (18446744073709551615UL)) {
    ...
    for (a = 0, i = start; i != end + incr; i += incr) {
        ...
        if (!ctx->filled && idx >= ctx->begin) {
            ...
            if (ctx->circa[idx] == (18446744073709551615UL)) {
                ...
                terms[a++] = enif_make_ulong(env, ctx->circa[idx]);
            }
        }
    }
...
save(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    *((typeof(60)*)(bin_data + i)) = 60; i += sizeof(60);;
...
static ErlNifFunc functions[] =
{
 ...
 ErlNifEntry* nif_init(void); ErlNifEntry* nif_init(void) { static ErlNifEntry entry = { 2, 2, "ecirca_large", sizeof(functions) / sizeof(*functions), functions, &init, ((void *)0), ((void *)0), ((void *)0), "beam.halfword" }; ; return &entry; }
